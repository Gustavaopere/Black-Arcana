...
- `provider-catalog-runtime-registry-probe.md` — removable server-side QA companion that observes bounded Iron's spell registry IDs and effective `school` / `enabled` / `allow_crafting` through the exact public 3.16.3 host API on the assembled pack. It is isolated from production and never creates an automatic catalog PASS.
