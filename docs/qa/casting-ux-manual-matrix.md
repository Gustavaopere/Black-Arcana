...
| Input persistence | Rebind radial/cast/quick slots, restart the client, then reuse them | Rebound mappings persist across the normal client restart and remain usable | ⬜ PENDING |
...
| Result correlation | Authoritative result arrives while another spell is currently selected | A matched result is positively attributable to its emitted `castId` context, not the current selection; an unmatched result remains generic rather than inventing a spell identity | ⬜ PENDING |
...
| Client authority isolation | Change Black Arcana presentation/config preferences and repeat equivalent server-owned cases for every gameplay-authority dimension applicable to the exact campaign | For each applicable dimension exercised and recorded — cost, cooldown, progression, target admission, Arcane Danger and world settlement where legitimately available — client-only preferences/presentation do not alter server-owned outcomes; HUD/radial/forecast interaction also cannot mutate underlying hazard/gate/resource state | ⬜ PENDING |
