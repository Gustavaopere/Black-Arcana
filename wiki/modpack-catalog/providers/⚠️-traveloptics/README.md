...
- current sibling presence: `ABSENT / REPOSITORY-WIDE MODLIST SEARCH`;
...
There is no current-pack promotion gate while Traveloptics is absent from the physical sibling modlist. If the provider is reintroduced, the audit must restart from the new physical artifact rather than inheriting the old alpha as current authority. At that time, re-evaluate:
