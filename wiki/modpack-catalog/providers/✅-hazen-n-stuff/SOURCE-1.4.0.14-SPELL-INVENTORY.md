# Hazen N Stuff 1.4.0.14 — source-pinned spell inventory
...
| # | Family / school authority | Registry ID | Display name | Concrete class | Min rarity | Max level | Special catalog gate |
|---:|---|---|---|---|---|---:|---|
| 1 | Ender — Iron's | `hazennstuff:endraconic_meteor` | Endraconic Meteor | `EndraconicMeteorSpell` | EPIC | 5 | host/default |
| 2 | Blood — Iron's | `hazennstuff:violent_regurgitation` | Violent Regurgitation | `ViolentRegurgitationSpell` | EPIC | 5 | host/default |
| 3 | Blood — Iron's | `hazennstuff:bone_bolt` | Bone Bolt | `BoneBoltSpell` | COMMON | 10 | host/default |
| 4 | Fire — Iron's | `hazennstuff:soul_flaming_strike` | Soul Flaming Strike | `SoulFlamingStrikeSpell` | RARE | 5 | host/default |
| 5 | Fire — Iron's | `hazennstuff:soul_flame_bolt` | Soul Flame Bolt | `SoulflameBoltSpell` | COMMON | 10 | host/default |
...
The only provider-side `canBeCraftedBy` specialization affecting registered spells is inherited through provider abstract bases:

| Spell | Provider gate | Source-pinned acquisition evidence |
|---|---|---|
| `golden_shower` | Golden Shower Spellbook must be present in inventory | exact release packages a crafting recipe for `hazennstuff:golden_shower_spellbook` carrying the spell |
...
Exact Hazen 1.4.0.14 resources add provider-owned items to focus tags used by the relevant school authorities:

| School authority | Focus path populated by Hazen |
|---|---|
...
Hazen's exact release adds the Hydro focus tag to Iron's `school_focus`.

The current physical HazentouveLib 1.0.9 line is source-correlated to `Hazentouvel/HazentouveLib@641acf4e9e254f1af9f59b2eb2251ff0f1fcfc08`; that source adds the Cosmic, Shadow and Radiance focus tags to Iron's `school_focus`.

This closes the custom-school focus route at catalog level. Exact assembled-pack host config and runtime behavior remain separate QA.
...
### Later branch content

Later `1.21.1` source contains spell changes not present in the release pin, including `coruscated_discharge`. Those later identities/changes are not projected into the installed 1.4.0.14 inventory.

## Clean-room boundary

The source is used only to record factual catalog/interoperability facts. Black Arcana does not retain or reproduce provider method bodies, recipe ingredients, formula implementations, assets, localization prose or other copyrighted implementation material.
