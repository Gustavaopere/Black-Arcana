...
A reconstrução canônica após Phase 2BK fecha **1250 objetos mágicos semânticos**. Phase 2BL fecha Goety Iron 3.1 em **+14** e Goety Cataclysm 1.21.1-1.8.2 em **+52**, portanto o mínimo passa a **1316**. Phase 2BM fecha Ars Polymorphia 1.0.3 como bridge de resolução de conflitos de receita com **+0** magias semânticas independentes, Phase 2BN fecha Ars Sable 1.1.2 como bridge/infra espacial Ars Nouveau ↔ Sable com **+0**, Phase 2BO fecha Farmer's Spell 'n Spellbooks 1.0.5.1 com **+6 `COUNTED_SOURCE_PINNED`**, Phase 2BP fecha SnackPirate's Aeromancy Additions 1.2.8 com **+10 `COUNTED_SOURCE_PINNED`**, e Phase 2BQ fecha Ars Nouveau: Two-Way Portals 2.0.0 como `ZERO_SEMANTIC_PORTAL_INFRA` com **+0**. O mínimo corrente permanece **1332**. O denominador global continua incompleto e nenhuma porcentagem semântica é declarada.
...
- mínimo estrito global após a reconciliação Phase 2BQ: **1332**;
...
## Phase 2BQ — Ars Nouveau: Two-Way Portals 2.0.0 — componente #65 / semantic +0

| Mod ID | Artefato físico | Estado |
|---|---|---|
| `ars_two_way_portals` | `ars_two_way_portals-2.0.0.jar` | EXACT HASH-MATCHED ARTIFACT / 25 PROVIDER CLASSES / 2 ITEMS / 3 RECIPES / 7 REQUIRED COMMON MIXINS / EXACT NEOFORGE+ARS+IMMERSIVE DEPENDENCY RANGES / ZERO SPELL-GLYPH-RITUAL-RITE-ABILITY SURFACE / `ZERO_SEMANTIC_PORTAL_INFRA` / +0 / COMPONENT #65 / CURRENT-HOST RUNTIME QA FAIL-CLOSED |

Evidence: physical SHA-1 `233846fc30667893c5f36a719da576d5eed43f5c`; CurseForge project/file `1599062 / 8515817`; NON-MERGE PR #222 exact artifact run `34735280002` GREEN with text artifact `10311071402`; durable PR #223 merge `main@bc5428b5855e4d5821d5bd901fb591a62ecf3cbe`; post-merge CI #2622 / run `34735586680` GREEN; QA artifact `10310957237`. Exact metadata targets `immersive_portals_core [6.0.7,7.0.0)`, resolving the old source-baseline mod-id uncertainty. Runtime pair lifecycle, configs, mixin application, persistence, frame/Weave behavior and optional Immersive assembled-host behavior remain fail-closed.

## Phase 2BP — SnackPirate's Aeromancy Additions 1.2.8 — componente #64 / semantic +10

| Mod ID | Artefato físico | Estado |
|---|---|---|
| `aero_additions` | `aero_additions-1.2.8.jar` | EXACT PHYSICAL IDENTITY / EXACT OFFICIAL SOURCE PIN `ae282b32...` / 10 ACTIVE UNCONDITIONAL WIND SPELLS / 5 COMMENTED REGISTRATIONS EXCLUDED / `COUNTED_SOURCE_PINNED` / WIND+BREEZE-ROD SCROLL-FORGE ROUTE / 2 REQUIRED COMMON MIXINS / 3 PROVIDER PAYLOAD REGISTRATIONS OBSERVED / +10 / COMPONENT #64 / CURRENT-HOST RUNTIME QA FAIL-CLOSED |

### Evidence boundary

- physical SHA-1 `dee32c9fa84d6e39846608f8f77591ea56f`; physical CurseForge hash `3079423735`;
- exact official source `snackerpirater/aero-additions@ae282b32d25ad76ef8d01c637ec05566a767ae4c`;
- exact source tree `fcee08e613fbfa030f034268a0240c8693ab7f45`, recursive `truncated=false`;
- exactly ten active provider spell identities: `wind_charge`, `updraft`, `airstep`, `asphyxiate`, `feather_fall`, `wind_shield`, `airblast`, `wind_blade`, `flush`, `dash`;
- Tornado, Thunderclap, Summon Breeze, Telelink and Shapeshift are excluded because their `registerSpell(...)` lines are commented out at the audited pin;
- provider Wind school is taxonomy/support and is not counted as an eleventh semantic action;
- provider data places `minecraft:breeze_rod` in the Wind/host school-focus path; Iron's 3.16.3 source-line Scroll Forge contract supplies host-native focus-to-school/spell selection;
- provider global loot modifiers append Breeze Rod support to the Trial Chamber normal-vault reward route; Updraft Tome and Wind Sword embed already-counted spells and add zero semantic identities;
- mixin footprint: 2 required common bindings, `ServerPlayerMixin` and `ServerPacketListenerMixin`;
- observed provider network surface: protocol/version string `1.0.0` with S2C `DashParticlesPacket`, C2S `AirstepPacket` and S2C `AASyncPlayerDataPacket`; provider remains authority for its movement/state handling;
- no `src/test` subtree exists in the exact source tree;
- source build targets NeoForge `21.1.228` and Iron's `1.21.1-3.16.1`; physical pack uses NeoForge `21.1.248` and Iron's `3.16.3`; declared Iron's range `[1.21.1-3.15.0,1.21.1-4.0.0)` accepts the physical version but does not prove assembled runtime behavior;
- client/server boot, required mixin application, physical config/tag behavior, Scroll Forge path, representative casts, payload sync, persistence/reload and duplicate-processing remain fail-closed direct QA gates;
- Black Arcana does not duplicate Iron's/Aeromancy casting, mana, cooldown, projectile/effect or provider-state pipelines;
- clean-room: factual identity/registry/dependency/network/acquisition evidence only; no upstream implementation/assets/localization text copied or adapted.

Phase 2BP therefore promotes only source-pinned catalog identity/count/reachability and component closure. It does **not** create a Black Arcana Wind school, alternate mana/scroll pipeline or assembled-pack runtime compatibility PASS.
...
- exact official source `baileyholl/ars-sable@1fd83f3a998e3a41b5a21d0d6529140a0b0a55ba`, commit message `1.1.2`;
- exact source role is compatibility/spatial infrastructure over Ars Nouveau + Sable sublevels, not a standalone spell system;
- mixin footprint: 24 common plus 5 client bindings, all required by `ars_sable.mixins.json`;
- provider network surface: registrar protocol `2`; zero provider-owned payload registrations observed in the exact registration function; generic Ars packet helper transport exists and is not reclassified as an Ars Sable payload catalog;
- source/storage/warp/Planarium/Mob Jar/entity/pathfinding/camera/render surfaces are bridge behavior over provider-owned systems, not independent semantic spell identities;
...
- exact source built against Sable `1.2.2`, while physical Sable is `2.0.5`; exact source built against Ars Nouveau `5.11.7.1354`, while physical Ars is `5.13.1`; 29 required direct mixin targets keep ABI/mixin compatibility as runtime QA;
- upstream GameTests document intended Planarium/storage/tracking/warp scenarios at the source pin, but do not prove the current physical host combination;
- exact source metadata declares `LGPLv3`, while the root `LICENSE` contains The Unlicense/public-domain dedication; this conflict is recorded without inferring reuse rights;
- client/full-pack behavior, multiplayer isolation, reload/reconnect persistence, chunk/assembly lifecycle and installed-host interop remain fail-closed;
- clean-room: no upstream implementation body/assets/localization text are copied or adapted.

Phase 2BN therefore promotes only the provider-component catalog closure. It does **not** add semantic magic, create a Black Arcana spatial/sublevel runtime or certify the physical host combination as runtime-compatible.
...
| 63 | 2BO / #214 | `farmers_spell` | CANÔNICO em `main@34a5fd49...`; +6 `COUNTED_SOURCE_PINNED`; post-merge CI #2565 GREEN; runtime host QA fail-closed |
...
Phase 2BF / Somake, Phase 2BG / Leylines, Phase 2BH / Goety, Phase 2BL / Goety addons, Phase 2BM / Ars Polymorphia, Phase 2BN / Ars Sable, Phase 2BO / Farmer's Spell, Phase 2BP / Aeromancy Additions e Phase 2BQ / Ars Nouveau: Two-Way Portals já foram reconciliadas no nível de evidência descrito acima. Não reiniciar esses fechamentos a partir de branches antigas.
...
Os estados correntes desta reconciliação passam a **1332 objetos semânticos mínimos / 65 de 100 componentes**. A promoção só se torna canônica na `main` após merge desta reconciliação e exact-SHA post-merge CI GREEN.
