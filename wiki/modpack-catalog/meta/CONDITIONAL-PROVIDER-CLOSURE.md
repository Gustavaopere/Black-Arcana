...
- Somake: preserve the exact 1.0.8-fix 67-ID registry and host-default audits as historical evidence only; the physical provider has changed to 1.0.9. The broad Legendary Monsters presence gate is publisher-confirmed for its Spirit/Red Soul/items tranche and satisfied at the latest explicit provider-specific physical checkpoint, but current object-level registry predicates, physical byte equality and host/reachability conclusions still require revalidation before promotion; do not erase or reinterpret the older checkpoint as current;
