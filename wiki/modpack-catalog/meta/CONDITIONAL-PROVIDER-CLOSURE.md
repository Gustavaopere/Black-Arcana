...
Status: `6 PROVIDERS ⚠️ / CLOSURE PATHS EXPLICIT / NO STATUS OR COUNT PROMOTION IMPLIED`
...
|---|---|---|---|
| Asterism Arcanum `1.21.1-0.1.0` | 11 exact registered spell identities known; 10 ordinary survival spells already counted; `TrailblazeSpell` excluded because registration is commented | Effective survival reachability of exact active `asterismarcanum:astral_gateway`, whose publisher intent is creative-only/unfinished but static defaults may expose provider-native acquisition unless deployed config/datapack blocks it | [`⚠️-asterism-arcanum/SURVIVAL-CLOSURE-CHECKLIST.md`](../providers/⚠️-asterism-arcanum/SURVIVAL-CLOSURE-CHECKLIST.md) |
...
| Mobstein `5.4.4` | Exact physical artifact authority closed; publisher-facing catalog materialized; machine/entity/resource/modifier/downstream-effect surfaces semantically excluded under the global metric from publisher evidence | Residual finite boundary: determine whether the four documented syringe-triggered behaviors (Reviver, Lightningbolt, Mobstenio Blood, Suspicious) or any provider-exposed active surface has an independent Mobstein-owned action identity, then resolve current-pack activation only for any identity actually proven | [`⚠️-mobstein/REGISTRY-CLOSURE-CHECKLIST.md`](../providers/⚠️-mobstein/REGISTRY-CLOSURE-CHECKLIST.md) |
| Not Enough Glyphs `4.6.1` | 40 registrations established from exact current-pack source; `momentum` source-disabled; 39 source-enabled candidates identified with their Ars SERVER config paths | Effective deployed `[general].enabled` state for the 39 candidates from the actual server/world configuration | [`⚠️-not-enough-glyphs/DEPLOYED-CONFIG-CHECKLIST.md`](../providers/⚠️-not-enough-glyphs/DEPLOYED-CONFIG-CHECKLIST.md) |
| Somake Spells `1.0.8-fix` | Exact hash-matched artifact; 67/67 registry identities closed; optional registration gates closed; retained exact class audit proves no Somake-level `allowCrafting`/`isEnabled` override across the 67 and exact Iron's `3.16.3` host defaults are enabled + craftable; 67/67 `.guide` keys exist | Effective deployed Somake `enableSpellLockSystem`, effective Iron's per-spell/global/datapack `enabled` + `allow_crafting` state, and usable school-focus/acquisition paths in the actual pack; source defaults and guide-key existence are not deployed reachability | [`⚠️-somake-spells/SURVIVAL-REACHABILITY-CHECKLIST.md`](../providers/⚠️-somake-spells/SURVIVAL-REACHABILITY-CHECKLIST.md) |
| T.O Magic n' Extras / Traveloptics `4.4.0.1-1.21.1` | 33 publisher-release registered spell identities cataloged by verified school; 32 residual localization-only IDs excluded; unique/weapon spell structure audited | Four finite gates: actual physical artifact/patch identity; loot-modifier registry initialization on assembled pack; `traveloptics:blackout` survival acquisition; Somake Aqua ↔ T.O Aqua coexistence/authority | [`⚠️-traveloptics/CLOSURE-CHECKLIST.md`](../providers/⚠️-traveloptics/CLOSURE-CHECKLIST.md) |

## Evidence priority

For these six providers, future work must use the following order:
...
- Mobstein: exact physical identity, publisher-facing object-level card tranches and the public semantic exclusion of machine/entity/resource/modifier/downstream-effect surfaces; only the residual active-action boundary remains;
- Not Enough Glyphs: source registration inventory and fallback namespace resolution;
- Somake: 67/67 exact registry identities, optional registration gates and the 67-class host-default eligibility audit; do not re-audit class-level `allowCrafting` unless the physical provider/host version changes;
