...
| Not Enough Glyphs `4.6.2` | Current physical line revalidated; exact source pin `45604dd1…` declares 4.6.2. The registration blob and `EffectMomentum` blob are byte-identical to the audited 4.6.1 pin, so the 40-registration / 39-source-enabled matrix carries forward unchanged; exact publisher File `8880291` is fingerprinted at SHA-1 `32eea2c478a346ee7499f6a0db156241116f73e9` and embeds Sauce `0.0.50.97`; installed-JAR byte equality is not claimed | Effective deployed `[general].enabled` state for the 39 candidates from the actual server/world configuration | [`⚠️-not-enough-glyphs/DEPLOYED-CONFIG-CHECKLIST.md`](../providers/⚠️-not-enough-glyphs/DEPLOYED-CONFIG-CHECKLIST.md) |
