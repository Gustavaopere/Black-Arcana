...
| Somake Spells `1.0.9` | Current physical filename/runtime identified by the sibling modlist; exact publisher release file `8867079` resource-audited at SHA-1 `171841ac9f802be9309ecc166c1d972ac6d404c0`; release metadata/resource surface closed; 83 base spell localization roots have matching `.guide` keys, but localization is not registry proof; the exact 1.21.1 changelog also names 16 spells (10 Spirit/Evocation, 1 Holy, 1 Sound, 1 Aqua, 3 Blood) and explicitly replaces Summon Zombie with Summon Drowned, without closing registry IDs; official 1.0.9 changelog confirms Legendary Monsters as optional and gates its Spirit/Red Soul/items tranche on provider presence; the latest explicit sibling Legendary Monsters dossier records `legendary_monsters` 2.2.2 installed at its preserved physical checkpoint; historical 1.0.8-fix 67-ID exact audit retained only as an older checkpoint | Prove physical-pack byte equality to the audited 1.0.9 release; close the exact **deployed** 1.0.9 registry/optional-registration outcomes through provider evidence or deterministic exact-pack registry observation; keep generalized predicate internals fail-closed unless provider-authoritative evidence exists; revalidate Somake config/lock behavior, Iron's host gates, school-focus/acquisition and survival reachability on the assembled pack | [`⚠️-somake-spells/CURRENT-1.0.9-REVALIDATION-CHECKLIST.md`](../providers/⚠️-somake-spells/CURRENT-1.0.9-REVALIDATION-CHECKLIST.md) |
...
- Somake: preserve the exact 1.0.8-fix 67-ID registry and host-default audits as historical evidence only; the physical provider has changed to 1.0.9. The broad Legendary Monsters presence gate is publisher-confirmed for its Spirit/Red Soul/items tranche and satisfied at the latest explicit provider-specific physical checkpoint. Current-pack registration outcomes may be reclosed by exact assembled-server registry observation, but physical byte equality, host/reachability conclusions and any generalized predicate claims remain separate; do not erase or reinterpret the older checkpoint as current;
