...
Historical publisher file `6342780` closes **33 unique registered `traveloptics:<id>` spell identities**, 33 field→class→ID mappings and zero registry initializer branches for that audited release. Current sibling `neoforge-rpg-skilltree@d809c7c2e617f5ee14f6867af618c52922d85589` contains no Traveloptics dossier/path/mod-id/filename reference, so this row is provenance only and is not a current denominator/blocker. The same exact alpha carries 32 additional root localization spell IDs that are not registered and are excluded. Provider ancestry proves `AbstractUniqueSpell.allowCrafting() = false` and `AbstractWeaponSpell.allowCrafting() = true`; nine of ten Unique registrations have direct structured loot anchors, while `traveloptics:blackout` remains object-level survival-unresolved.
...
Current sibling authority `neoforge-rpg-skilltree@d809c7c2e617f5ee14f6867af618c52922d85589` preserves the current provider lines used here; Traveloptics is absent from that current modlist/dossier tree.
...
|---|---|---|---|
| [Somake Spells](../providers/⚠️-somake-spells/README.md) 1.0.9 | current sibling modlist identifies `somakespells-1.0.9-1.21.1.jar`; exact publisher file `8867079` resource-audited at release SHA-1 `171841ac9f802be9309ecc166c1d972ac6d404c0`; 83 base spell-localization roots + 83 matching `.guide` keys bound the current resource surface; exact 1.21.1 publisher changelog separately names 16 spells (10 Spirit/Evocation, 1 Holy, 1 Sound, 1 Aqua, 3 Blood) and replaces Summon Zombie with Summon Drowned, but public names are not substituted for registry identities; official 1.0.9 changelog closes only the broad Legendary Monsters presence gate for its Spirit/Red Soul/items tranche, and the latest explicit sibling dossier records `legendary_monsters` 2.2.2 installed at its preserved checkpoint; historical 1.0.8-fix exact registry was 67 IDs | `CONDITIONAL / CURRENT REGISTRY REVALIDATION REQUIRED / +0` | physical-pack byte equality to the audited release is not yet proven; 83 localization roots are not substituted for registry entries; the broad Legendary Monsters presence condition does not establish object-level registrations; exact 1.0.9 registry/remaining optional predicates, deployed config/host gates and survival acquisition/reachability remain open |
...
3. reclose Somake 1.0.9 current-line physical equality, exact registry/object-level optional gates and then survival/config reachability; the broad Legendary Monsters presence subgate is already publisher-confirmed at the preserved physical checkpoint, while the historical 1.0.8-fix 67-ID audit must not be promoted as current.
