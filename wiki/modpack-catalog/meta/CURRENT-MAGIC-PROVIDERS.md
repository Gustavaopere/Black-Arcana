...
**Cross-domain note:** Gaze 1.1.7.1 remains ⚠️ in the global magic-provider catalog because it owns one counted spell and 26 config-conditional rites, but its current sibling dossier lives under `PROJECT-INSTRUCTIONS/modlist/Addons/`; it is therefore not part of the 45-row physical `Magic` category count. Ender's Spells and Stuff: Requiem 0.1.7 is likewise cross-domain under `Addons/`, but is ✅ cataloged with 53 strict semantic actions. Reliquified Iron's Spells 'n Spellbooks 0.2.7 is confirmed directly by the current physical Project Library modlist and is now ✅ cataloged with 25 source-pinned provider-owned relic ability roots; it likewise changes the global semantic numerator without changing the 45-row physical-`Magic` subtotal. Iron's Spellbooks KubeJS 4.0.3 remains a separate cross-domain magic-capable component under a root-level dossier because current pack scripts can create content outside the bridge namespace.
...
- ✅ Reliquified Iron's Spells 'n Spellbooks 0.2.7 — current physical SHA-1 `e22fd94c87cb88b3cfa4637c48a1058260d35a9b`; exact 0.2.7 source closes **25 provider-owned relic ability roots** across 23 ability-bearing relic classes, all with the provider's Iron's-structure chest loot route; `COUNTED_SOURCE_PINNED`, **+25**.
...
O mínimo semântico estrito corrente é **1521**. O denominador semântico final e o denominador técnico cross-domain continuam abertos; não publicar percentual global.
