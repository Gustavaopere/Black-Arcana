...
A métrica principal para o usuário é a cobertura de objetos mágicos semânticos — spells, glyphs/spell-parts, rituais/rites e equivalentes discretos. Seu denominador global ainda está em reconstrução; portanto nenhuma porcentagem final é declarada aqui. Farmer's Spell acrescenta **+6 `COUNTED_SOURCE_PINNED`**, elevando o mínimo estrito de **1316 para 1322**; Aeromancy Additions acrescenta mais **+10 `COUNTED_SOURCE_PINNED`**, elevando-o para **1332**; Ars Polymorphia, Ars Sable, Ars Nouveau: Two-Way Portals, Vampire Spells Addon, Mobstein e Acolyte são fechamentos semânticos **+0**; GTBC's Geomancy Plus acrescenta **+12 `COUNTED_RELEASE_BOUNDED`**, elevando o mínimo estrito para **1344**. Hazen N Stuff 1.4.0.14 acrescenta **+38 `COUNTED_SOURCE_PINNED`**, elevando o mínimo estrito corrente para **1382**. Acolyte não altera esse mínimo.
...
## Provider freshness override — Acolyte 1.0.3

O sibling revalidado em `neoforge-rpg-skilltree@278b427023136d7c43d85dc188d0eac1ac85ef3a` confirma `acolyte-1.0.3.jar`, mod id `acolyte` e runtime `1.0.3` como componente mágico atual dependente de Iron's.

O CurseForge file exato `8098233` foi auditado clean-room no nível estrutural. O artefato possui 93 classes do provider, **zero** classes em cadeia de herança até Iron's `AbstractSpell`, **zero** resources de spell próprios e **zero** identities de localization de spell próprias. Ele referencia explicitamente 31 spells já pertencentes ao Iron's e também usa seleção dinâmica do host. Essas referências descrevem loadout/uso, não novas identidades de Acolyte.

Consequência canônica: **✅ Acolyte — componente spellcaster/encounter catalogado; 0 spells semânticos próprios; +0 semantic objects**. O mínimo semântico reconstruível permanece **1382**. O provider também está fora do antigo conjunto técnico congelado, reforçando que o denominador global permanece `PENDING REBASE`. Igualdade byte-a-byte do JAR físico com o publisher e runtime QA assembled-pack continuam fail-closed.
