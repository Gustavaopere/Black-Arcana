...
Physical JAR: `farmers-spell-n-spellbook-1.0.5.1-1.21.1.jar`
...
Physical Farmer's Delight host: `FarmersDelight-1.21.1-1.3.4.jar`
Physical GeckoLib host: `geckolib-neoforge-1.21.1-4.9.2.jar`
...
- exact current physical provider identity, SHA-1 and CurseForge hash from the 595-entry physical modlist;
...
The source-pinned registry contributes **6 independent semantic spell objects** under `SEMANTIC-MAGIC-COVERAGE.md`:
...
2. Source build uses Farmer's Delight `1.3.2`; the physical pack uses `1.3.4`.
3. Source build and physical pack both use GeckoLib `4.9.2`; version equality is recorded but does not replace runtime testing.
4. Seven direct required mixins bind common and client/render behavior.
5. Physical Iron's is `3.16.3`, matching the public host source line used for the narrow SchoolType/Scroll Forge contract by version label; no cryptographic source/binary equivalence is claimed.
6. Client boot, dedicated-server boot, mixin application, Foodgeist progression, Scroll Forge acquisition and representative execution of all six spells remain assembled-pack runtime QA gates.
7. A loader/dependency range being accepted is not a substitute for direct host validation.
