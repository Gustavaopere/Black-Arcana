...
- 24/09 current physical-Magic reconciliation — sibling `neoforge-rpg-skilltree@e3689bbc04ba54be1612a13c242496330e99df06` preserves the current 587-entry physical dossiers. The literal category-path set now contains **45** rows whose path includes `Magic`; after Apokinetics exact closure all 45 map to provider directories at **40 ✅ cataloged / 5 ⚠️ partial-conditioned**. Gaze was reclassified by the sibling from `Addons + Magic/` to `Addons/`; it remains a ⚠️ magic-relevant provider in the global catalog and is excluded only from this literal folder-path count. The strict reconstructible semantic minimum remains **1392** because Apokinetics is exact-zero semantic. The technical denominator remains `PENDING REBASE`.
...
- [`meta/PROVIDER-AUDIT-QUEUE-DELTA-APOKINETICS.md`](meta/PROVIDER-AUDIT-QUEUE-DELTA-APOKINETICS.md) — closes Apokinetics 1.0.6 as an exact hash-matched machine-augmentation provider with 12/12 Machine Gem capability types and zero provider-owned spell/glyph/ritual identities; runtime machine/config QA remains fail-closed.
...
- [`meta/CAPABILITY-MATRIX-DELTA-APOKINETICS.md`](meta/CAPABILITY-MATRIX-DELTA-APOKINETICS.md) records the exact machine-gem/socket/Pylon augmentation overlap and its zero spell/glyph/ritual semantic disposition.
