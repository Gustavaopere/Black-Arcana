...
- [`meta/PROVIDER-AUDIT-QUEUE-DELTA-ACOLYTE.md`](meta/PROVIDER-AUDIT-QUEUE-DELTA-ACOLYTE.md) — prevails for `acolyte`, recording exact publisher-release structural closure as an Iron's spellcaster component with **+0** provider-owned semantic spells; installed-byte equality and assembled-pack runtime QA remain open.
...
- Acolyte `1.0.3` — current physical JAR/mod id/runtime certified by the sibling 22/09 re-audit; exact CurseForge file `8098233` structural audit finds 93 provider classes, zero `AbstractSpell` subclass chain, zero provider spell resources/localization identities and 31 explicit Iron's-owned host spell references plus dynamic host selection; classified **+0 semantic**, with physical hash equality and assembled-pack AI/config/trade/recruitment QA still fail-closed;
