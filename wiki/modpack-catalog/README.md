...
- 24/09 current physical-Magic reconciliation — sibling `neoforge-rpg-skilltree@e3689bbc04ba54be1612a13c242496330e99df06` preserves 587 top-level entries; the current category layout has **45 rows whose physical category path contains `Magic`**, all mapped to provider directories after ownership-name normalization. Effective state is **40 ✅ cataloged / 5 ⚠️ partial-conditioned** after the exact Apokinetics 1.0.6 zero-semantic closure. Gaze remains a separate ⚠️ magic provider in the global catalog even though its sibling dossier is currently categorized under `Addons/`, outside this physical-category count. Companions +9 and Crystal Chronicles +1 keep the strict reconstructible minimum at **1392**; Apokinetics and Cataclysm 1.1.14 contribute no new semantic delta in this reconciliation. The technical denominator remains `PENDING REBASE`.
