...
- 24/09 current physical-Magic reconciliation — sibling `neoforge-rpg-skilltree@e54536b2c875d7b60edc27669473d1d8a71eaf84` preserves the current 587-entry physical provider dossier set; the category layout has **45 rows whose physical category path contains `Magic`**, all mapped to provider directories after ownership-name normalization. Effective state remains **41 ✅ cataloged / 4 ⚠️ partial-conditioned**: Tombstone gains 10 counted release-bounded actions but remains partial because additional config-sensitive castables are still unresolved. Gaze remains a separate ⚠️ magic provider in the global catalog even though its sibling dossier is categorized under `Addons/`, outside this physical-category count; Traveloptics also remains a global partial outside this subtotal. Companions +9, Crystal Chronicles +1, Relics +41 and Tombstone +10 move the strict reconstructible minimum to **1443**. The technical denominator remains `PENDING REBASE`.
...
- [`meta/PROVIDER-AUDIT-QUEUE-DELTA-CORAIL-TOMBSTONE.md`](meta/PROVIDER-AUDIT-QUEUE-DELTA-CORAIL-TOMBSTONE.md) — prevails for Tombstone 9.5.6; **10 `COUNTED_RELEASE_BOUNDED` prayer/Ritual-Flute actions** are now strict-counted, while additional config-sensitive castable magic-item actions remain conditional.
...
The physical `Magic` category is fully mapped to provider directories; **four** rows in that physical category remain partial/conditioned after Relics closure. Gaze and Traveloptics remain separate global partial providers outside that subtotal. Future provider checkpoints must therefore be selected from the current unresolved queue rather than from stale historical blockers.
