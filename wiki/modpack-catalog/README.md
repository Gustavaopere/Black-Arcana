...
- [`meta/PROVIDER-AUDIT-QUEUE-DELTA-COMPANIONS.md`](meta/PROVIDER-AUDIT-QUEUE-DELTA-COMPANIONS.md) — prevails for `companions`, closing the 1.3.4 Magic Book semantic surface as nine source-pinned player actions while assembled-pack runtime QA remains fail-closed.
...
- Companions! `1.3.4` — current physical JAR/SHA-1 is certified at sibling physical order #104; exact official source pin `95c9445...` closes nine unconditional provider-owned Magic Book action identities. Six are reachable through tamed Minion coin interactions, Black Hole/Magic Ray through vanilla chest-table injection and Naginata through a provider recipe; Soul Mage reuse is deduplicated and runtime QA remains fail-closed;
