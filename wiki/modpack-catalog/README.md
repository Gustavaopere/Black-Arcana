...
- 24/09 current physical-Magic reconciliation — sibling `neoforge-rpg-skilltree@dd83e06b25d9a4ab70d5f1bf0702d8375b388242` preserves the current 587-entry physical provider dossier set; the category layout has **45 rows whose physical category path contains `Magic`**, all mapped to provider directories after ownership-name normalization. Effective state is **41 ✅ cataloged / 4 ⚠️ partial-conditioned** after the exact Relics 0.12.8 ability/synergy closure. Gaze remains a separate ⚠️ magic provider in the global catalog even though its sibling dossier is categorized under `Addons/`, outside this physical-category count; Traveloptics also remains a global partial outside this subtotal. Companions +9, Crystal Chronicles +1 and Relics +41 move the strict reconstructible minimum to **1433**. The technical denominator remains `PENDING REBASE`.
