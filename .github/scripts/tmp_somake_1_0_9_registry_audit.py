...
def modspells_bootstrap_detail(data, max_offset=45):
    cp,utf,cls,string,ref,fields,methods,utf8=parse_class(data)
    clinit=next((m for m in methods if m[0]=="<clinit>"),None)
    if not clinit or clinit[3] is None:
        return []
...
def inspect(label, blob, expected_sha1):
    ...
    with zipfile.ZipFile(io.BytesIO(blob)) as zf:
        ...
        if label=="CURRENT_1_0_9":
            bootstrap=modspells_bootstrap_detail(zf.read(MODSPELLS),45)
            ...
            phoenix_read=target_method_detail(zf,"com/somake/somakespells/config/StartupFeatures.class","readSacredPhoenixBlessing")
