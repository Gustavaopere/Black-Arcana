...
def gate_map_modspells(data):
    ...
    for idx,(off,op,raw) in enumerate(ins):
        if op==0x12:
            ...
            if rr:
                owner,name,desc,tag=rr
                call_events.append((idx,off,owner,name,desc,last_string))
                if owner and "DeferredRegister" in owner and name=="register":
    ...
    gates=[]
    for idx,off,owner,name,desc,last_string_at_call in call_events:
        mod=None
        source=None
        if owner and owner.endswith("/ModList") and name=="isLoaded":
            mod=last_string_at_call
            source="ModList.isLoaded"
        elif name=="isLoaded" and owner in helper_to_mod:
            mod=helper_to_mod[owner]
            source=owner.split("/")[-1]+".isLoaded"
        if not mod:
            continue
        branch=None
        for j in range(idx+1,min(idx+5,len(ins))):
            boff,bop,braw=ins[j]
            if bop in (0x99,0x9a):
                branch=(j,boff,bop,braw)
                break
        if not branch:
            gates.append((mod,source,off,None,None,[]))
            continue
        j,boff,bop,braw=branch
    ...
    for mname,mdesc,macc,code in methods:
        ...
        for idx,(off,op,raw) in enumerate(ins):
            ...
            if not rr:
                continue
            owner,name,desc,tag=rr
            if name!="define" or "Ljava/lang/String;Z" not in desc:
                continue
            ...
            for k in range(max(0,idx-10),idx):
                ...
                if pop==0x12:
                    s=string(praw[1])
                    if s is not None: key=s
                elif pop in (0x13,0x14):
                    s=string(struct.unpack_from(">H",praw,1)[0])
                    if s is not None: key=s
                elif pop==0x03:
                    default=False
                elif pop==0x04:
                    default=True
            rows.append((mname,key,default,owner,desc))
    return rows
...
def inspect(label, blob, expected_sha1):
    ...
    with zipfile.ZipFile(io.BytesIO(blob)) as zf:
        ...
        for mod,source,off,branchop,targetoff,ids_in_fallthrough in gates:
            print(f"SOMAKE_{label}_REGISTRATION_GATE={mod}|source={source}|call_offset={off}|branch={branchop}|target={targetoff}|fallthrough_ids={','.join(x or '<none>' for x in ids_in_fallthrough)}")
        config_name="com/somake/somakespells/config/Config.class"
        if config_name in zf.namelist():
            defs=config_boolean_definitions(zf.read(config_name))
            for mname,key,default,owner,desc in defs:
                if key=="enableSpellLockSystem":
                    print(f"SOMAKE_{label}_SPELL_LOCK_BOOLEAN_DEFINE={mname}|key={key}|default={default}|owner={owner}|desc={desc}")
