...
def refs(text):
    ...
    for line in text.splitlines():
        ...
        m = re.search(r'// Method ([A-Za-z0-9_/$]+)\.(?:"([A-Za-z0-9_$<>]+)"|([A-Za-z0-9_$<>]+)):', line)
        if m:
            method_refs.add(f"{m.group(1).replace('/', '.')}.{m.group(2) or m.group(3)}")
...
def instructions(method):
    ...
    for line in method["lines"]:
        ...
        if not m:
            continue
        inst.append((int(m.group(1)), m.group(2), (m.group(3) or "").strip(), (m.group(4) or "").strip()))
    return inst
...
def method_from_comment(comment):
    m = re.search(r'Method ([A-Za-z0-9_/$]+)\.(?:"([A-Za-z0-9_$<>]+)"|([A-Za-z0-9_$<>]+)):', comment)
    return None if not m else f"{m.group(1).replace('/', '.')}.{m.group(2) or m.group(3)}"
        ...
        for i, (off, op, operand, comment) in enumerate(ins):
            ...
            if fallthrough_refs:
                findings.append({
                    ...
                    "inference": "true_skips_guarded_fallthrough" if branch[1] == "ifne" else "false_skips_guarded_fallthrough",
                })
    return findings
...
def loaded_mod_guards(methods, mod_id, guarded_needles):
    ...
    for method in methods:
        ins = instructions(method)
        for si, item in enumerate(ins):
            if string_from_comment(item[3]) != mod_id:
                continue
            load_index = None
            for j in range(max(0, si - 5), min(si + 10, len(ins))):
            ...
            if load_index is None:
                continue
            branch = None
            for j in range(load_index + 1, min(load_index + 5, len(ins))):
                if ins[j][1] in ("ifeq", "ifne") and ins[j][2].isdigit():
                    branch = ins[j]
                    break
            if not branch:
            ...
            if refs_between:
                findings.append({
                    ...
                    "inference": "loaded_false_skips_guarded_fallthrough" if branch[1] == "ifeq" else "loaded_true_skips_guarded_fallthrough",
                })
    return findings
...
def config_default(code_text):
    candidates = []
    for method in split_methods(code_text):
        ins = instructions(method)
        for i, item in enumerate(ins):
            ...
            bool_const = None
            has_define = False
            for _, op, operand, comment in ins[i:i+12]:
                if op == "iconst_0":
                    bool_const = False
                elif op == "iconst_1":
                    bool_const = True
                ref = method_from_comment(comment)
                if ref and ref.endswith("ModConfigSpec$Builder.define"):
                    has_define = True
                    break
...
setup_family = [m for m in split_methods(progress) if "setupEntries" in m["header"]]
setup_text = "\n".join("\n".join(x["lines"]) for x in setup_family)
...
result["rite_disable_gate"] = branch_guard(mod_methods, "Config.disableGazeRites", ("GazeRiteRegistry", "GazeSpiritRiteEffectTypes"))

irons = javap("com.strawberry.gaze.compat.irons_spellbooks.IronsCompat", code=True)
irons_methods = split_methods(irons)
f, m, s = refs(irons)
result["irons_compat_refs"] = {"spell_registry_refs": [x for x in f + m if "SpellRegistry" in x], "strings": s}
result["irons_loaded_gate"] = loaded_mod_guards(
    irons_methods,
    "irons_spellbooks",
    ("SpellRegistry", "IronsEntities", "GazeIronsSpellBookLoader", "ENCYCLOPEDIA_UNVEILED"),
)
...
with zipfile.ZipFile(JAR) as z:
    ...
    result["irons_class_hits"] = [n for n in names if any(t in n.lower() for t in ("soulward", "encyclopedia_unveiled")) and n.endswith(".class")]
    resource_hits = []
    for name in names:
