...
## D030 — Current danger coefficients are committed-cast base acquisition
Stage 05A schema version 1 exposes `corruptionCoefficient` and `strainCoefficient` but does not yet serialize damage-linked acquisition, unavoidable floors, strain bonus multipliers, or strain hard-gate thresholds. Until a later schema version explicitly adds those fields, `corruptionCoefficient` means base Corruption units acquired by a successfully committed dangerous cast and is mitigated only by the independently snapshotted Corruption Resistance channel; `strainCoefficient` means base Arcane Strain units acquired by that committed cast and is not implicitly reduced by Arcane Resistance. No per-damage Corruption/Strain, unavoidable floor, backlash/corruption strain bonus, or strain hard gate may be inferred from these two coefficients alone. Both resistance/state preflights are frozen before normal resource reservation and persistent state is materialized only by the terminal hazard commit; cancelled or failed casts do not acquire either state.

## D031 — Deferred final validation does not block downstream implementation
Missing manual, real-modpack, representative-performance, migration-fixture or exact-release-head evidence blocks validation/release claims, not downstream implementation, when predecessor runtime contracts are frozen and applicable automated gates are green. Stages 06→09 integrate sequentially through the latest `main`. Deferred rows remain explicit and cannot be inferred as PASS. Deterministic engineering tests and CI remain mandatory during implementation. Stage 09 remains `RELEASE BLOCKED` until the accumulated final validation campaign is executed on the exact release candidate.

## D032 — Forbidden Domains default to bounded in-world fields
Stage 07.06 Forbidden Domains default to bounded localized fields/arenas in an existing loaded dimension rather than dynamically created dimensions/instances. The in-world model must reuse canonical world-border, protection, loaded-chunk, destination-recovery and `WorldEffectPolicy` contracts and enforce hard radius, duration, entity and restoration budgets. Dynamic temporary dimensions require a separate explicit architectural decision supported by a requirement the in-world model cannot satisfy.

## D033 — Arcane polarity, fusion and metamagic are a Stage 07A cross-domain layer
Stage 07A is inserted after Stage 07 and before Stage 08 so white/black/neutral polarity, bounded spell fusion and metamagic can be resolved across all canonical spell domains before progression/balance freezes their costs and gates. Polarity is derived from validated source and agency semantics rather than damage type: self-owned or explicitly consensual sourcing is Luminal/white, non-consensual extraction/coercion/parasitism is Umbral/black, and non-extractive manipulation may remain Arcane/neutral. Polarity is independent from Arcane Danger/forbidden severity and must not create a second Corruption or Strain channel. Fusion and metamagic transform one server-owned cast plan before ordinary commit and must converge on the existing canonical cast transaction: one cast identity, replay claim, target resolution, composite resource transaction, cooldown, hazard transaction, world-effect admission and progression/reward causality identity. Provider-native composition remains preferred where it already safely expresses the mechanic; optional provider failures are fail-closed unless an explicit identity-preserving fallback exists. Mahou Tsukai remains reference-only under D002/D003: no Mahou mana-growth system, recipes, scrolls, staves, self-cutting activation, code, assets, text, models, sounds or animations may be imported, and its observed mechanics may only be reimplemented independently with original Black Arcana identity/presentation or routed through separately verified provider contracts.

## D034 — Required validation blocks stage promotion
D034 supersedes D031 for numbered-stage progression. The canonical promotion order is `05 -> 05A -> 06 -> 07 -> 07A -> 08 -> 09`. A stage may advance only after every requirement defined by its own plans is complete: implementation, integration, automated tests, required manual/physical/real-client/real-modpack/provider-native validation, persistence/lifecycle/multiplayer/world-safety checks, and acceptance criteria as applicable. A frozen runtime contract, green CI, an exact-SHA QA artifact, or a status such as `IMPLEMENTED`, `FINAL VALIDATION DEFERRED`, `PHYSICAL VALIDATION PENDING`, `PARTIAL` or `CANONICAL RUNTIME` is supporting evidence only and does not authorize promotion when required evidence remains missing.

Existing downstream code created under D031 remains repository history and may be reused after its predecessor stages close, but it is not evidence that the intervening stages are complete and must not drive current implementation priority. The active stage remains the earliest incomplete stage. Required manual evidence may never be inferred from automation. A numbered plan file receives the `✅-` prefix only after its own complete acceptance contract is proven. Later stages must not be used to defer functional work or required validation from an earlier stage.


## D035 — Final real-client validation is consolidated in Stage 09
D035 supersedes D034 only for the timing of manual/physical/real-client/real-modpack/provider-native observation. The canonical implementation order remains `05 -> 05A -> 06 -> 07 -> 07A -> 08 -> 09`, and functional implementation, provider integration, persistence/lifecycle/world-safety contracts, deterministic acceptance tests and applicable CI remain mandatory in the originating plan before promotion.

A numbered plan may receive the `✅-` prefix and the next implementation stage may begin when its runtime/integration contract is implemented, its deterministic acceptance is complete, and every still-required physical/manual observation has been explicitly enumerated and transferred to the Stage 09 final-validation matrix. Transferred rows remain `PENDING / DEFERRED TO STAGE 09`; they are never inferred as PASS from automation.

Stage 09 owns one consolidated exact-release-candidate campaign for accumulated real-client, real-modpack, representative-performance, migration and provider-native observations. Stage 09 remains `RELEASE BLOCKED` until every release-blocking transferred row is directly evidenced. A physical failure reopens the originating plan/stage for the smallest authority-preserving fix, deterministic regression coverage where possible, and repetition of the affected final-validation rows.

This decision does not permit functional work, known defects, missing integrations or deterministic failures to be deferred to Stage 09. It changes validation scheduling only.
