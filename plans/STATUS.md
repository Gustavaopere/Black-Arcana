...
| 07 Spell Domains | 🟦 IN PROGRESS — 07.01–07.04 CANONICAL; 07.05 IMPLEMENTED / AWAITING MERGE | 07.05 PR #54 technical head `6e021ef...` passed workflow `34004647369` with 92/92 GameTests + dedicated smoke; exact final-head and post-merge main gates still required; 07.06–07.07 pending; real-modpack/manual acceptance deferred |
...
07.05 Black Flame was rebuilt sequentially from `main@0e508b646b602beefd136bf9602945e247b2a524` in PR #54. It adds operation-specific requested `WorldMutationClass` admission under the existing worst-case `WorldEffectProfile`, a separate bounded provider-neutral `WorldMutationProtectionAdapterRegistry`, protected TEMPORARY and LIMITED/PERMANENT CAS settlement routes, and the server-owned `black_arcana:black_pyre` runtime. The frontier is bounded to radius 12, 256 seen cells, 16 processed candidates per tick, 8 concurrent frontiers and 1,200 ticks; unloaded candidates are dropped and no vanilla fire/random-tick propagation or force-load path is used. Entity damage stays on canonical `EntityInteractionType.DAMAGE` with dedupe, same-level checks and settlement-time reauthorization. Lifecycle hardening verifies stop/start clears ephemeral frontier state while Stage 04 restoration ownership remains independent. Pre-documentation technical head `6e021ef28ebb450a84a7dff7fa8a545f52a90389` passed workflow `34004647369` with **92/92 required GameTests** and dedicated-server smoke. This is automated pre-merge evidence only; PR #54 still requires final exact-head GREEN after documentation and exact-SHA post-merge `main` validation before 07.05 becomes canonical. Malum amplification and real Iron's/Malum host acceptance remain fail-closed/deferred where no verified causal/provider evidence exists.

07.06 Forbidden Domains through 07.07 Familiars & Divination remain pending and must continue sequentially only after 07.05 is canonical. They are not pulled in through stale PR #22 ancestry. Optional provider/host behavior that requires the real modpack remains part of the deferred final validation campaign.
