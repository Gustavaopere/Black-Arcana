...
| 07 Spell Domains | 🟦 IN PROGRESS — 07.01 + 07.02 + 07.03 CANONICAL; 07.04 ON PR #52 | 07.04 implemented and under final PR-head validation; 07.05–07.07 pending; real-modpack/manual acceptance deferred |
...
07.04 Space & Displacement is implemented on PR #52 from canonical `main@f30999b375d42506127eb4c7570f1b8bfd68262c`. It contains Threshold Gate, Veilstep Reflex, Anchor Recall, Reciprocal Transposition, Vector Reversal and one shared bounded safe-destination authority. Hard ceilings cap Gate throughput at 32 transfers/s, Recall age at 600 ticks and range at 128 blocks, Transposition at 16 swaps/s, resulting reversal speed at 2.5 blocks/tick and safe-position search at 64 candidates. Runtime state is owner/server scoped and lifecycle cleanup is wired into the live NeoForge game bus. A wiring audit exposed missing composition-root registration for four stateful runtimes and reproduced it with an explicit RED contract before fixing `BlackArcanaMod`. A second RED contract reproduced raw `setPos(...)` settlement in player-capable Gate/Transposition paths before both settlement and rollback were moved to `teleportTo(...)`. Destination admission remains loaded-chunk/no-force-load, live-world-border, collision/headroom, fluid, dimension, protection and vehicle aware. The domain is not canonical until the final PR-head workflow is GREEN, PR #52 is merged and exact-SHA post-merge CI publishes the canonical QA artifact. Real-modpack/provider/manual host acceptance remains deferred under D031.

07.05 Black Flame through 07.07 Familiars & Divination remain pending and must continue sequentially from the latest canonical `main` after 07.04 is promoted. They are not pulled in through stale PR #22 ancestry. Optional provider/host behavior that requires the real modpack remains part of the deferred final validation campaign.
