...
- **07.07 Familiars & Divination** — `IN PROGRESS`. PR #72 merged the bounded server Noetic/familiar/gaze/sanctuary substrate at `5c818c12bb6f580893e44f31fd0e17b9c1fe5840`; final head `673aff57e15ec29a6fc0d6a94f0034726b99a4c1` and exact-SHA post-merge CI were GREEN with 103/103 GameTests and dedicated-server smoke. Production camera/HUD/input and the complete per-spell specification gate are still missing for Astral Severance/Borrowed Sight, so 07.07 is **not** promoted as a completed domain and Stage 08 must not use it as canonical balance input yet.
...
Stage 07 implementation is canonical through **07.06**. Stage 07.07 remains in progress until its missing production mechanics and per-spell specifications are completed and reviewed. Stage 08 Progression & Balance must not start from 07.07 as canonical balance input before that gate closes.
