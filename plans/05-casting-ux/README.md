...
- [`09-keyboard-focus-navigation.md`](09-keyboard-focus-navigation.md) — implemented bounded client-local keyboard focus/navigation for radial and loadout screens while preserving mouse behavior, server authority and current global keybinding contracts; direct real-client acceptance remains deferred;
...
- `KeyboardFocusNavigation` is a client-only deterministic helper for radial/loadout focus initialization, traversal and page reconciliation. It has no networking, persistence, provider or cast authority.
- `CastingUxSemantics` maps already-authorized client/server presentation facts into surface-neutral focus, admission/result, hazard and loadout-membership roles. The radial consumes independent selected/hovered/focused/composite roles with non-color markers; the loadout editor distinguishes keyboard focus, synchronized accepted membership and local draft additions/removals; the HUD distinguishes authoritative denial, effect failure and success without converting forecasts into cast results. This layer has no gameplay admission or provider authority.
...
- broader semantic non-color-only state cues beyond the implemented selected/hovered/focused and accepted/draft markers;
...
`09-keyboard-focus-navigation.md` now records the implemented bounded screen-navigation layer from PR #157. Radial focus uses canonical slot indices with Tab/Shift+Tab same-page wrap and selection-only Enter/Space activation; the loadout editor uses absolute row focus with Up/Down same-page clamp and Space-only local draft toggling. Existing page, Enter-apply and Delete/Backspace-clear semantics are preserved. The helper and screen state remain client-local and do not create a second cast or accepted-loadout authority path. Direct real-client keyboard/accessibility/coexistence validation remains deferred.
...
- Stage 05.09 followed two explicit RED→GREEN cycles. RED `dc457ad711ba02e9e14bdc04d3a61e69752d8425` failed before `KeyboardFocusNavigation` existed; helper GREEN `c7f196daf5d0737b371f6e23b40db7b7885821b7` closed the pure navigation contracts. RED `9032aaae705cb426e59f056a1293a01c81a22dca` then failed before radial/loadout screen wiring; implementation head `8fcc90fc4b5e5b82519d63c5a1d2a93b543820da` passed the full pipeline. Reviewed semantic/test head `3e560b202a0fd35630fd366293d35c7ad03ca31e` passed workflow `34403555530`. Final reconciled PR head `4f451a67d471f9373ff64050f5284a66e0869fd1` passed both push workflow `34405021764` and PR workflow `34405028047` through JUnit, diff sanity, NeoForge build, built-JAR verification, Foundation GameTests and dedicated-server smoke. Canonical merge `29a0099e899e03d80bf904c2d5ead72f40421fe8` passed exact-SHA post-merge workflow `34405514309` through the same complete gate and published artifact `black-arcana-29a0099e899e03d80bf904c2d5ead72f40421fe8` (artifact ID `10125267266`, SHA-256 `de2da7faf18971c9f6e06b7b5896b0390c03c3a9b058b549f7b72eb6d5ec6030`).
