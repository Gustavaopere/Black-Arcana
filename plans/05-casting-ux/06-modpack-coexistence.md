...
| Iron's Spells 'n Spellbooks | `irons_spellbooks` | `1.21.1-3.16.3` | external spell/resource/casting provider |
...
These facts establish presence/version only. They do not establish a public integration API.
...
Therefore this Stage 05 plan may define coexistence behavior and test requirements, but any future direct technical bridge still requires exact-version API/source verification before code is written.
...
External mods remain authority for their own:
...
Black Arcana must not mirror, mutate or reinterpret those systems merely because they are visible on the same screen.
...
- avoid taking ownership of external action bars;
...
Black Arcana does **not** currently claim an integration with it.

### 6.2 Deduplication rule

Black Arcana must not add a second persistent panel that reproduces Spell Actionbar's provider-owned information for Iron's spells.

In particular, Black Arcana should not mirror external-provider:
...
Black Arcana may still show a concise **Black Arcana-authored** denial/risk/context line when a Black Arcana cast operation requires it.

### 6.3 Future bridge rule

If a future requirement asks Black Arcana spells to appear inside Spell Actionbar:

1. verify exact `spell_actionbar 1.1.4` API/source/docs;
2. verify whether it exposes a supported third-party registration boundary;
...
Without a verified safe seam, the integration is rejected and the two UIs remain separate.

## 7. Iron's Spells coexistence

Iron's `1.21.1-3.16.3` is an external magic provider with its own spell/resource semantics.
...
- Black Arcana invocation surfaces remain optional and do not replace provider-native casting for provider-owned spells;
- any Black Arcana adapter must preserve provider resource/cooldown authority and exactly-once processing.
...
The current repository search did not identify a Black Arcana-owned `epicfight` integration surface.
...
- no duplicate cast from one physical input;
...
For every future external invocation surface, the planning checklist is:

1. identify the provider that owns the item/action;
2. verify an exact-version supported event/API boundary;
3. convert the provider action into one bounded Black Arcana cast intent/reference;
4. preserve one canonical `ArcanaCastId` root;
5. reserve/commit costs exactly once;
6. preserve server targeting and cooldown ownership;
7. avoid provider-native cast plus Black Arcana cast both firing from one action;
8. define recursion/deduplication protection;
9. define fallback when the provider is absent;
10. test dedicated-server classloading and optional-dependency failure.

No thematic similarity is enough to create this bridge.
...
| Spell Actionbar | external action bar visible + BA denial | bounded BA denial remains readable without taking provider authority |
...
- one physical input can trigger duplicate Black Arcana casts;
...
- external provider resource is double-debited;
...
2. exact provider API/source/docs are verified;
3. the boundary preserves causal identity and exactly-once processing;
4. the integration can fail safely;
5. regression tests and real-client coexistence evidence are defined before merge.
...
- authority and deduplication rules are documented;
- the exact-version API verification gate is explicit for any future bridge;
- real-pack coexistence scenarios are defined;
