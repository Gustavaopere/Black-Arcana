...
| Cataclysm: Spellbooks | installed `1.1.13-1.21`; JAR `cataclysm_spellbooks-1.1.13-1.21.jar`; CurseForge File ID `8792628`; separate public source baseline `AceTheEldritchKing/Cataclysm_Spellbooks_1.21.1@82a0af71f051058fe515c8b1cb9168e7f972f41c` still declares `1.1.11-1.21` | `REFERENCE_ONLY / COMPATIBILITY_TARGET` exact-artifact identity/current publisher-scale audit plus read-only stale-source semantic baseline for deduplication | current CurseForge and source `TEMPLATE_LICENSE.txt` identify PolyForm Shield 1.0.0, while source `gradle.properties` declares `All Rights Reserved`; source metadata is therefore not treated as a derivation grant. PolyForm Shield also contains a noncompete restriction. No code/assets are copied/adapted; matching 1.1.13 source/registry/API remains `REVIEW_REQUIRED` and fail-closed |
| Leyline Spellbooks | installed `1.0.3`; JAR `leylines-1.0.3.jar`; CurseForge File ID `8565076`; physical modlist SHA-1 `dfa6908731f432905caaaa1e53b4aedeaa26ed59` | `REFERENCE_ONLY / COMPATIBILITY_TARGET` publisher-public audit of Leyline school identity, nine signature-spell names as a lower bound, pillar/rift progression and 1.0.3 encounter rules | project is **All Rights Reserved**. No exact publisher source revision was located and inspectable binary bytes were not obtained in the permitted environment. The previously recorded `5307a4edc885ab949eed4438d9d7f9cb6176421d` is retired from Leylines provenance because it belongs to a different artifact rather than the current physical Leylines row. No source/JAR bytecode or assets were copied, decompiled or adapted; registry/API internals remain `REVIEW_REQUIRED` and fail-closed |
| Somake Spells | installed `1.0.8`; JAR `somakespells-1.0.8-1.21.1-fix.jar`; CurseForge File ID `8417850`; physical SHA-1 `b0ad94c1504709662bee2d08700375ccecbb5ec7` | `REFERENCE_ONLY / COMPATIBILITY_TARGET` exact-release plus publisher-current/changelog audit of Somake spell families, Elemental Charges, Aqua/Symmetry, tier books/Grimoires/Upgrade Forge, Soul Fire/Infernal Fire rituals and equipment | project is **All Rights Reserved**. No publisher source revision matching 1.0.8-fix was located. No source/JAR bytecode or assets are copied, decompiled or adapted. Complete registry/API is `REVIEW_REQUIRED`/fail-closed. The pack also contains deprecated T.O Magic 1.21.1 alpha; Somake's future Aqua-migration statement does not authorize or prove runtime authority transfer to that artifact |
...
| Goety | installed `3.1.4`; JAR `goety-3.1.4.jar`; physical SHA-1 `a0770e180e4e8b1b87d8fa9c8356e9dbf34d82a7`; public `Vivideru/Goety-3` checkpoints `4230e3bce2842779a6667ae6e5bfef8f53a27541` (3.1.0) and `6c41a04f2d712097c4461f969a6bb8ee277149ef` (3.1.1); `ModItems.java` blob `db3c63b366803e2d46aa4a996b5bb0f358437a7f` stable at both checkpoints | `REFERENCE_ONLY / COMPATIBILITY_TARGET` factual current-line catalog/deduplication evidence: 123 active Focus item registrations in the audited public source interval; exact 3.1.4 JAR/source equivalence, semantic reachability/deduplication, ritual identities and supported integration seams remain open | upstream `LICENSE.txt` is **mixed-license**: original `src/main/java/com/Polarice3/` code is stated MIT, while `src/main/java/com/Vivideru/` additions are All Rights Reserved unless separately stated. Audited `ModItems.java` is within the MIT-scoped path and was consulted read-only for factual IDs/counts/blob/version provenance only. No upstream code/assets/text are copied/adapted; the whole source tree is not represented as MIT and exact 3.1.4 implementation/API claims remain `REVIEW_REQUIRED`/fail-closed |
...
| Goety Iron | installed `3.1`; CurseForge File ID `8662179` | `REFERENCE_ONLY / COMPATIBILITY_TARGET` for Goety↔Iron's servant bridge | project page declares MIT, but no exact official source revision was located during the audit. Public description/changelog may be cataloged; internal/API claims remain `REVIEW_REQUIRED` until source/API evidence is pinned |
