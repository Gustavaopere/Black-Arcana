...
| Mobstein | installed `5.4.4`; JAR `mobstein-5.4.4-neoforge-1.21.1.jar`; CurseForge project `1193873`, exact file `8040734` | `REFERENCE_ONLY / COMPATIBILITY_TARGET` publisher-guide audit for resurrection, bodies/organs, constructed experiments, companions, structures/Witherstein and 5.4.4 Sable-compatibility QA planning | project is **All Rights Reserved** and no official public source/API revision was located. No JAR decompilation or upstream code/assets/text copying; player-facing publisher documentation is catalog evidence only. Provider-specific internals/API integration remain `REVIEW_REQUIRED`/fail-closed |
| Monsters & Spellbooks | installed `0.0.16.3`; JAR `monstersspellbooks-0.0.16.3.jar`; CurseForge project `1428928`, exact file `8788560`; publisher-linked repository head `RedReaper28/Monsters-Spellbooks-1.21.1@1ab9b72...` remains metadata version `0.0.14` | `REFERENCE_ONLY / COMPATIBILITY_TARGET` audit for Necro/curses/summons/gear, Aero-removal correction and Iron's/Irons Jewelry compatibility QA planning | current CurseForge project displays MIT, while the linked source metadata declares `All Rights Reserved` and does not match installed version. The root `TEMPLATE_LICENSE.txt` covers only NeoForged MDK template files. No provider implementation internals are promoted from the stale source; exact 0.0.16.3 source/API derivation remains `REVIEW_REQUIRED`/fail-closed until version/license reconciliation |
