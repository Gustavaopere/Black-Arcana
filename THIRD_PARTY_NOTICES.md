...
| Malum | installed `1.8.2`; bounded source line from `SammySemicolon/Malum-Mod@f56691e56e591a6d8d1859ff119e749375e14d61` through `03b743a37f3eeb0cc7f4364f0730e1f135f78408`; read-only inspection of `MalumSpiritRiteTypes.java`, supporting `MalumSpiritRiteEffectTypes.java`, `MalumGeasEffectTypes.java`, `MalumSpiritTypes.java` and their path histories | optional spirit/soul `DEPENDENCY_API / COMPATIBILITY_TARGET`; factual catalog/deduplication uses whole-interval registry identity evidence for 26 base rites, a supporting distinct-effect check for the two special rites, 37 active Geas effect types and 9 Spirit types | **license conflict remains unresolved:** CurseForge declares GNU LGPLv3, while exact `gradle.properties` on the audited line declares `All Rights Reserved` and no root LICENSE resolves the discrepancy. Registry source was consulted read-only for factual names/counts/path history/blob identity/deduplication only; the supporting Rite-effect registry contributes no additional semantic-action objects. No upstream code/assets are copied/adapted. Source-derived implementation/API specification and provider-resource mutation remain `REVIEW_REQUIRED`/blocked and fail-closed |
