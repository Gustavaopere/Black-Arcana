...
/** Non-destructive Stage 03 probe that pays Black Arcana costs from Ars player mana. */
public final class ArsSyntheticContent {
    public static final long COOLDOWN_TICKS = 40L;

    private ArsSyntheticContent() { }

    public static void install(
        ArcanaServerRuntime runtime,
        ArsManaAccess manaAccess,
    ...
    public static ArcanaSpellDefinition definition() {
        return new ArcanaSpellDefinition(
            ArsIntegrationIds.PROBE_ARCANA_ID,
            "spell.black_arcana.ars_integration_probe",
            "black_arcana:textures/gui/spell_icons/ars_integration_probe.png",
            new ArcanaCost(ArsManaCostProvider.RESOURCE_ID, ArsIntegrationIds.PROBE_MANA_COST),
            false);
    }
    ...
    private static void installDefinition(ArcanaServerRuntime runtime, ArcanaSpellDefinition definition) {
        ...
        if (existing != null) {
            if (!existing.equals(definition)) {
                throw new IllegalStateException("Ars integration spell id already has a different definition");
            }
            return;
        }
    ...
    private static void installCooldown(ArcanaServerRuntime runtime) {
        ...
        cooldowns.put(
            ArsIntegrationIds.PROBE_ARCANA_ID,
            new ArcanaCooldownSpec(ArsIntegrationIds.PROBE_ARCANA_ID.canonical(), COOLDOWN_TICKS, false));
