...
public final class ReciprocalTranspositionGameTests {
    ...
    public static void twoLivingEndpointsSwapAtomicallyAfterFreshRevalidation(GameTestHelper helper) throws Exception {
        ...
        Object result = swap(server, owner.getUUID(), first.getUUID(), second.getUUID(), firstVersion, secondVersion,
            server.getTickCount(), true, true);
    ...
    public static void staleEndpointOrDeniedConsentMutatesNeitherEndpoint(GameTestHelper helper) throws Exception {
        ...
        Object stale = swap(server, owner.getUUID(), first.getUUID(), second.getUUID(), firstVersion, staleSecondVersion,
            server.getTickCount(), true, true);
        ...
        Object consent = swap(server, owner.getUUID(), first.getUUID(), second.getUUID(), freshFirst, freshSecond,
            server.getTickCount() + 1L, false, true);
    ...
    public static void throughputLimitRejectsSecondSuccessfulSwapInSameWindow(GameTestHelper helper) throws Exception {
        ...
        Object firstSwap = swap(server, owner.getUUID(), first.getUUID(), second.getUUID(),
            snapshotVersion(server, first.getUUID()), snapshotVersion(server, second.getUUID()), now, true, true);
        ...
        Object overflow = swap(server, owner.getUUID(), first.getUUID(), second.getUUID(),
            snapshotVersion(server, first.getUUID()), snapshotVersion(server, second.getUUID()), now, true, true);
    ...
    public static void itemEntityEndpointSwapsWithoutCloningOrConsumingStack(GameTestHelper helper) throws Exception {
        ...
        ItemEntity item = new ItemEntity(level, living.getX() + 4.0D, living.getY(), living.getZ(), new ItemStack(Items.DIAMOND, 3));
        ...
        Object result = swap(server, owner.getUUID(), living.getUUID(), item.getUUID(),
            snapshotVersion(server, living.getUUID()), snapshotVersion(server, item.getUUID()),
            server.getTickCount(), true, true);
    ...
    private static Object swap(MinecraftServer server, UUID ownerId, UUID firstEntityId, UUID secondEntityId,
            long firstVersion, long secondVersion, long nowTick, boolean firstConsent, boolean secondConsent) throws Exception {
        Class<?> runtime = Class.forName(RUNTIME);
        Method method = runtime.getMethod("swap", MinecraftServer.class, UUID.class, UUID.class, UUID.class,
            long.class, long.class, long.class, boolean.class, boolean.class);
        return method.invoke(null, server, ownerId, firstEntityId, secondEntityId, firstVersion, secondVersion,
            nowTick, firstConsent, secondConsent);
    }
