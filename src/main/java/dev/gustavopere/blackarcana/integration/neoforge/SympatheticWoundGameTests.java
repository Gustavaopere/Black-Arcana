...
public final class SympatheticWoundGameTests {
    ...
    public static void mirroredDamageIsAttributedAndCannotRecurseThroughCrossLinks(GameTestHelper helper) throws Exception {
        ...
        ArcanaDecision firstLink = bind(server, new SympatheticWoundService.LinkSpec(
            first.getUUID(), second.getUUID(), now + 40L, 0.50D, 10.0D, 20.0D));
        ArcanaDecision secondLink = bind(server, new SympatheticWoundService.LinkSpec(
    ...
    @GameTest(template = "foundation_empty", timeoutTicks = 60)
    public static void playerTargetsAreDisabledByDefault(GameTestHelper helper) throws Exception {
        ...
        MinecraftServer server = helper.getLevel().getServer();
        long now = server.overworld().getGameTime();
        ArcanaDecision decision = bind(server, new SympatheticWoundService.LinkSpec(
    ...
    @GameTest(template = "foundation_empty", timeoutTicks = 60)
    public static void invulnerableTargetsUseCanonicalEntityAdmission(GameTestHelper helper) throws Exception {
        var caster = helper.spawnWithNoFreeWill(EntityType.COW, new BlockPos(2, 2, 1));
    ...
    private static ArcanaDecision bind(MinecraftServer server, SympatheticWoundService.LinkSpec spec) throws Exception {
        ...
        return (ArcanaDecision) bind.invoke(null, server, spec);
    }
}
