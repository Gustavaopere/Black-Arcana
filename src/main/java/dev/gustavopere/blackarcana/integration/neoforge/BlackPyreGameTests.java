...
public final class BlackPyreGameTests {
    ...
    public static void entityDamageRemainsIndependentWhenTerrainIsNotRequested(GameTestHelper helper) throws Exception {
        ...
        Object result = igniteDefault(helper.getLevel().getServer(), caster.getUUID(), List.of(target.getUUID()),
            3.0D, false, caster.blockPosition());
        ...
        helper.assertTrue("terrain_not_requested".equals(terrainCode(result)),
            "entity-only cast must expose terrain_not_requested");
        helper.succeed();
    }
    ...
    public static void cosmeticModeNeverMutatesBlocks(GameTestHelper helper) throws Exception {
        ...
        try {
            ...
            helper.assertTrue("black_pyre_cosmetic_only".equals(terrainCode(result)),
                "COSMETIC degradation must be explicit");
        } finally {
    ...
    public static void temporaryModeMutatesAndRestoresUnchangedCell(GameTestHelper helper) throws Exception {
        ...
        try {
            ...
            helper.assertTrue(helper.getLevel().getBlockState(seed).is(Blocks.STONE),
                "unchanged temporary cell must restore after expiry");
        } finally {
            runtime.configureWorldEffects(previous);
        }
        helper.succeed();
    ...
    public static void temporaryRestorationNeverOverwritesLaterWorldEdit(GameTestHelper helper) throws Exception {
        ...
        try {
            ...
            runtime.tick(now + 1_201L);
            helper.assertTrue(helper.getLevel().getBlockState(seed).is(Blocks.DIAMOND_BLOCK),
                "restoration CAS must not overwrite a later world/player edit");
        } finally {
            runtime.configureWorldEffects(previous);
        }
        helper.succeed();
    ...
    public static void mutationProtectionDenialDoesNotBurnWorldBudget(GameTestHelper helper) throws Exception {
        ...
        runtime.worldMutationProtectionAdapters().register(adapterId, query ->
            query.blockKey().packedBlockPos() == deniedPackedPos
        ...
        try {
            ...
            helper.assertTrue(runtime.worldEffectBudgets().usedUnits(castId) == 0,
                "protection denial must occur before world budget consumption");
    ...
    public static void limitedModeCommitsBoundedPermanentCell(GameTestHelper helper) throws Exception {
        ...
        try {
            ...
            runtime.tick(now + 1_201L);
            helper.assertTrue(helper.getLevel().getBlockState(seed).equals(committed),
                "LIMITED mutation must not enter temporary restoration tracking");
        } finally {
            runtime.configureWorldEffects(previous);
        }
        helper.succeed();
    ...
    public static void unloadedTerrainCandidateNeverForceLoadsChunk(GameTestHelper helper) throws Exception {
        ...
        helper.assertTrue(helper.getLevel().getChunkSource().getChunkNow(chunkX, chunkZ) == null,
            "far GameTest chunk must start unloaded");
        try {
            runtime.configureWorldEffects(new WorldEffectPolicyConfig(WorldEffectMode.TEMPORARY, 4096, true, Map.of()));
            ...
            helper.assertTrue(helper.getLevel().getChunkSource().getChunkNow(chunkX, chunkZ) == null,
                "Black Pyre must never acquire/load the rejected chunk");
        } finally {
            runtime.configureWorldEffects(previous);
        }
        helper.succeed();
    ...
    public static void blackPyreNeverDelegatesToVanillaFireCascade(GameTestHelper helper) throws Exception {
        ...
        try {
            ...
            helper.assertTrue(helper.getLevel().getBlockState(neighbor).is(Blocks.STONE),
                "neighbor cannot change through vanilla fire/random-tick cascade");
        } finally {
            runtime.configureWorldEffects(previous);
        }
        helper.succeed();
    ...
    private static Object igniteDefault(MinecraftServer server, UUID casterId, List<UUID> targets,
            double damage, boolean terrainRequested, BlockPos seed) throws Exception {
        Class<?> runtime = Class.forName("dev.gustavopere.blackarcana.integration.neoforge.MinecraftBlackPyreRuntime");
        Method method = runtime.getMethod("igniteDefault", MinecraftServer.class, UUID.class, List.class,
            double.class, boolean.class, int.class, int.class, int.class);
        return method.invoke(null, server, casterId, targets, damage, terrainRequested,
    ...
    private static int terrainCells(Object result) throws Exception {
        return (int) result.getClass().getMethod("terrainCells").invoke(result);
    }
