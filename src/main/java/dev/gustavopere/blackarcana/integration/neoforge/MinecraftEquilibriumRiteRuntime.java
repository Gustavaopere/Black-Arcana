 ...
 *
 * Endpoints must already be loaded living entities. The planner freezes one bounded
 * transfer from current authoritative health values; target policy is then revalidated
 * immediately before the paired health mutation. Hostile player and boss exchange are
 * disabled by default rather than inheriting the more permissive generic DAMAGE policy.
 */
public final class MinecraftEquilibriumRiteRuntime {
    ...
    public static TransferResult transfer(
        ...
        // Re-resolve and re-authorize immediately before mutation so unload/death/protection
        // changes cannot be bypassed by a stale endpoint snapshot.
        ...
        // Health mutations occur back-to-back on the authoritative server thread. The planner
        // guarantees sourceAfter + targetAfter equals the pre-settlement total and prevents
        // target overheal or source-floor violation.
